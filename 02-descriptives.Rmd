...
```
>Note: the `$` operator refers to a specific column relative to a specific data frame (i.e. collumn *sdc_age_0* in dataframe *globe_data*).
...
Again, the `epiDisplay` package provides a useful function to summarise categorical data. Running the `tab1()` function from this package gives you the frequency and proportion for each category and generates a histogram for a variable. Missing data are also shown in tables and histograms.
```{r eval=FALSE}
